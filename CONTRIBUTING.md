...
https://vscode.dev/github/LDU-IIPLab/LDU-IIPLab.github.io?vscode-lang=zh-cn

### 添加成员的个人页面

1. 在 [`content/authors`](content/authors) 目录下新建以自己名字命名的目录，放入 `_index.md` （内容参照其他人） 和头像 `avatar.jpg` 或 `avatar.png` （裁剪成正方形）。
...
1. 在 [`content/news`](content/news) 目录下新建以新闻日期命名的目录，放入 `index.md` （内容参照其他新闻）。
